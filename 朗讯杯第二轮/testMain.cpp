...
int MAP[21][21];
//队伍的程序名
char name[3][100] = {"","team12_program","team122_program"};
...
void writeStep(int x,int y){
    FILE *fp = fopen("step.txt","a");
    fprintf(fp,"%d %d\n",x,y);
    fclose(fp);
}
void memsetFile(){
    FILE *fp = fopen("step.txt","w");
    fclose(fp);
...
int main(){
    ...
    while (1){
        ...
        fscanf(fp,"%d%d",&x,&y);
        fclose(fp);
        cout << "read ans.txt done. position (" << x << " " << y << ")\n" ;
        writeStep(x,y);
		cout << "writeStep done. position (" << x << " " << y << ")\n" ;
        if (MAP[x][y]) {
            printf("(%d,%d) ..%s illegal output.",x,y,name[nowColor]);
            break;
        }
        ...
        writeBoard();
		cout << "writeBoard done. position (" << x << " " << y << ")\n" ;
